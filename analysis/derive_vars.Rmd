...
Optional - plots of alpha variables
```{r}
# plots
...
hilevel_uncorrected_counts <- higher_leveler(uncorrected_counts_df_list)
...
genus_analysis_list <- list(
  ...
  "uncorrected_high_level_vars" = hilevel_uncorrected_counts,
...
Add derived variables to phenodata for each data type
```{r}
...
imm_cor <- read_delim("~/Projects/tcga_micro/data/imm_cor_souce.tsv", delim = "\t") %>%
  rename(tcga_barcode = `TCGA Participant Barcode`)

# limma corrected counts
limma_hilevel_flat <- genus_analysis_list$limma_corrected_high_level_vars %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

limma_alpha_flat <- genus_analysis_list$limma_corrected_alpha %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

limma_pdata <- pData(genus_counts_es_list$limma_corrected_counts) %>%
  rownames_to_column(var = "tcga_barcode") %>%
  as_tibble() %>%
  left_join(limma_hilevel_flat) %>%
  left_join(limma_alpha_flat) %>%
  left_join(imm_cor) %>%
  print()

# cbseq corrected counts
cbseq_hilevel_flat <- genus_analysis_list$cbseq_corrected_high_level_vars %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

cbseq_alpha_flat <- genus_analysis_list$cbseq_corrected_alpha %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

cbseq_pdata <- pData(genus_counts_es_list$cbseq_corrected_counts) %>%
  rownames_to_column(var = "tcga_barcode") %>%
  as_tibble() %>%
  left_join(cbseq_hilevel_flat) %>%
  left_join(cbseq_alpha_flat) %>%
  left_join(imm_cor) %>%
  print()
  
# uncorrected counts
uncorrected_hilevel_flat <- genus_analysis_list$uncorrected_high_level_vars %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

uncorrected_alpha_flat <- genus_analysis_list$uncorrected_alpha %>%
  enframe() %>%
  unnest(value) %>%
  rename(tcga_proj = name) %>%
  print()

uncorrected_pdata <- pData(genus_counts_es_list$uncorrected_counts) %>%
  rownames_to_column(var = "tcga_barcode") %>%
  as_tibble() %>%
  left_join(uncorrected_hilevel_flat) %>%
  left_join(uncorrected_alpha_flat) %>%
  left_join(imm_cor) %>%
  print()

```

COMBATseq corrected counts
```{r}

# convert pdata to dataframe
cbseq_pdata_df <- cbseq_pdata %>%
  as.data.frame() %>%
  column_to_rownames(var = "tcga_barcode") 
# pdata variable annotation
v_metadata <- data.frame(
  labelDescription = colnames(cbseq_pdata_df),
  row.names = colnames(cbseq_pdata_df))
...
# get counts
counts_matrix <- exprs(genus_counts_es_list$cbseq_corrected_counts) 

# check matchy matchy
colnames(counts_matrix) %>% length()
rownames(phenoData) %>% length()

identical(colnames(counts_matrix), rownames(phenoData))

# create global annotation
annotation <- as.character("Raw counts from Agamemnon mode 2 summarized at genus level (or if genus is not available, at the lowest available taxonomic group at genus or above). Cases with no counts removed. Missing counts converted to zeros. Batch corrected with ComBatSeq using tumor type as experimental design and combination of sequencing center and plate (extracted from TCGA barcode) as batch effect. Summary indices, high-level taxonomic variables, and immune correlates included in phenodata.")

# assemble expressionset
cbseq_adj_es <- ExpressionSet(
  assayData = counts_matrix,
  phenoData = phenoData,
  annotation = annotation)
cbseq_adj_es

# save
saveRDS(cbseq_adj_es, "output/cbseq_genus_fullpdata_expressionset.rds")
...
# convert pdata to dataframe
limma_pdata_df <- limma_pdata %>%
  as.data.frame() %>%
  column_to_rownames(var = "tcga_barcode") 
# pdata variable annotation
v_metadata <- data.frame(
  labelDescription = colnames(limma_pdata_df),
  row.names = colnames(limma_pdata_df))
# make annotated phenoData
phenoData <- new("AnnotatedDataFrame", 
                 data = limma_pdata_df, 
                 varMetadata = v_metadata)
...
# get counts
counts_matrix <- exprs(genus_counts_es_list$limma_corrected_counts) 
...
# create global annotation
annotation <- as.character("Raw counts from Agamemnon mode 2 summarized at genus level (or if genus is not available, at the lowest available taxonomic group at genus or above). Cases with no counts removed. Missing counts converted to zeros. Batch corrected with limma using tumor type as experimental design and combination of sequencing center and plate (extracted from TCGA barcode) as batch effect. Summary indices, high-level taxonomic variables, and immune correlates included in phenodata.")

# assemble expressionset
limma_adj_es <- ExpressionSet(
  assayData = counts_matrix,
  phenoData = phenoData,
  annotation = annotation)
limma_adj_es
...
# save
saveRDS(limma_adj_es, "output/limma_genus_fullpdata_expressionset.rds")
...
```{r}
# convert pdata to dataframe
uncorrected_pdata_df <- uncorrected_pdata %>%
  as.data.frame() %>%
  column_to_rownames(var = "tcga_barcode") 
# pdata variable annotation
v_metadata <- data.frame(
  labelDescription = colnames(uncorrected_pdata_df),
  row.names = colnames(uncorrected_pdata_df))
# make annotated phenoData
phenoData <- new("AnnotatedDataFrame", 
                 data = uncorrected_pdata_df, 
                 varMetadata = v_metadata)
phenoData

# get counts
counts_matrix <- exprs(genus_counts_es_list$uncorrected_counts) 
...
# create global annotation
annotation <- as.character("Raw counts from Agamemnon mode 2 summarized at genus level (or if genus is not available, at the lowest available taxonomic group at genus or above). Cases with no counts removed. Missing counts converted to zeros. No batch correction. Summary indices, high-level taxonomic variables, and immune correlates included in phenodata.")

# assemble expressionset
uncorrected_adj_es <- ExpressionSet(
  assayData = counts_matrix,
  phenoData = phenoData,
  annotation = annotation)
uncorrected_adj_es
...
# save
saveRDS(uncorrected_adj_es, "output/uncorrected_genus_fullpdata_expressionset.rds")
