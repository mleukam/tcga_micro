...
Get read counts
```{r}
input_dir <- "/n/scratch3/users/m/mw292/AGAMEMNON/TCGA/Results2/TCGA_BRCA"
tcga_project = "BRCA"
...
gather_reads <- function(input_dir, tcga_project){
  ...
  paths_df <- dir(path = input_dir, 
      ...
      full.names = TRUE) %>%
    enframe() %>%
    dplyr::filter(grepl("hisat2_host.log", value)) %>%
    separate(value, into = c(NA, NA, NA, NA, NA, NA, NA, NA, NA, NA, "file_id"),
             sep = "/", remove = FALSE) %>%
    dplyr::select(pathto = value, file_id) %>%
    mutate(reads = map(pathto, ~ read.table(file = .,header = F, nrows = 1))) %>%
    unnest(cols = c("reads")) %>%
    mutate(project = rep(tcga_project, times = nrow(.))) %>%
    dplyr::select(project, file_id, reads = V1) 
    #
