...
imm_cor_combined_df <- dervars_combined %>%
  ...
  dplyr::select(-shannon, 
                ...
                -`Immune Subtype`,
                -`TCGA Subtype`,
                # select out missing data factors
                -`NA`,
                -`frac_NA`) %>%
  print()
...
write_xlsx(cleaned_spearman, "output/tcga_correlations_spearman.xlsx")
...
write_xlsx(cleaned_pearson, "output/tcga_correlations_pearson.xlsx")
...
global_cor_tab <- var_pairs %>% 
  mutate(
    r_test = purrr::map2(x, 
                         y, 
                         possibly(~ stats::cor.test(combined_global[[.x]],
                                                    combined_global[[.y]], 
                                                    method = "spearman"),
                                  otherwise = data.frame())),
    r_test = purrr::map(r_test, tidy)) %>%
...
combined_tbl %>% 
  .[["SKCM"]] %>%
  dplyr::select(pielou_evenness, til_regional_frac = `TIL Regional Fraction`) %>%
  ggplot(aes(x = pielou_evenness, y = til_regional_frac)) +
    geom_point() +
    geom_smooth() +
    ggtitle("Melanoma TIL regional fraction vs Pielou evenness")
```
